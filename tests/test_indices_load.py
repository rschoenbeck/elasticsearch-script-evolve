...
from es_script_agent.es.index import (
    LOANS_INDEX,
    USERS_INDEX,
    item_actions,
    setup_indices,
...
def test_setup_indices_drops_creates_and_loads(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("es_script_agent.es.index.bulk", fake_bulk)
...
def test_setup_indices_drops_existing_indices(
    ...
    monkeypatch.setattr(
        "es_script_agent.es.index.bulk", lambda c, a, **kw: (len(list(a)), [])
    )
...
def test_setup_indices_rejects_empty_adapter(
    ...
    es = FakeES()
    monkeypatch.setattr(
        "es_script_agent.es.index.bulk", lambda c, a, **kw: (0, [])
    )
